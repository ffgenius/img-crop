...
<template>
  <div class="demo-app">
    ...
    <aside v-if="!hideControls" class="control-panel">
      ...
      <div class="control-group">
        <label>图片</label>
        <select id="picture-select" data-testid="picture-select" :value="demoImage" @change="onImageChange">
      ...
      <!-- Reset -->
      <div class="control-group">
        <button data-testid="btn-reset" class="btn-reset" @click="resetAll">重置所有参数</button>
      </div>
      ...
      <div class="control-group crop-data">
        ...
        <table>
          <tbody>
            <tr><td colspan="2"><strong>百分比区域</strong></td></tr>
            <tr><td>X</td><td :id="'crop-area-x'">{{ croppedArea?.x != null ? Math.round(croppedArea.x) : '-' }}%</td></tr>
            <tr><td>Y</td><td :id="'crop-area-y'">{{ croppedArea?.y != null ? Math.round(croppedArea.y) : '-' }}%</td></tr>
            <tr><td>宽度</td><td :id="'crop-area-width'">{{ croppedArea?.width != null ? Math.round(croppedArea.width) : '-' }}%</td></tr>
            <tr><td>高度</td><td :id="'crop-area-height'">{{ croppedArea?.height != null ? Math.round(croppedArea.height) : '-' }}%</td></tr>
            <tr style="height:8px"><td colspan="2"></td></tr>
            <tr><td colspan="2"><strong>像素区域</strong></td></tr>
            <tr><td>X</td><td>{{ croppedAreaPixels?.x ?? '-' }}px</td></tr>
            <tr><td>Y</td><td>{{ croppedAreaPixels?.y ?? '-' }}px</td></tr>
            <tr><td>宽度</td><td>{{ croppedAreaPixels?.width ?? '-' }}px</td></tr>
            <tr><td>高度</td><td>{{ croppedAreaPixels?.height ?? '-' }}px</td></tr>
          </tbody>
        </table>
      </div>

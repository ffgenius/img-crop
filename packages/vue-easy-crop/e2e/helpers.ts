 ...
 */
export async function dragAndDropWithTouch(page: Page, selector: string, dx: number, dy: number) {
  await page.evaluate(
    ({ selector, dx, dy }) => {
  ...
  await page.evaluate(
    ({ selector, distance }) => {
      ...
      el.dispatchEvent(new TouchEvent('touchstart', {
        ...
        changedTouches: [_ct(el, finger1X, cy, 0), _ct(el, finger2StartX, cy, 1)],
        bubbles: true, cancelable: true,
      }))
      el.dispatchEvent(new TouchEvent('touchmove', {
        touches: [_ct(el, finger1X, cy, 0), _ct(el, finger2EndX, cy, 1)],
        changedTouches: [_ct(el, finger1X, cy, 0), _ct(el, finger2EndX, cy, 1)],
        bubbles: true, cancelable: true,
      }))
      el.dispatchEvent(new TouchEvent('touchend', {
        touches: [], changedTouches: [],
  ...
  await page.evaluate(
    ({ selector, rotation }) => {
      ...
      el.dispatchEvent(new TouchEvent('touchstart', {
        ...
        changedTouches: [_ct(el, f1X, f1Y, 0), _ct(el, f2StartX, f2StartY, 1)],
        bubbles: true, cancelable: true,
      }))
      el.dispatchEvent(new TouchEvent('touchmove', {
 ...
 */
export async function setViewportStable(page: Page, width: number, height: number) {
